...
`fmi3Pending` - is returned if the slave executes the function asynchronously.
...
Note, in Initialization Mode input variables can be set with `fmi3SetXXX` and output variables can be get
...
_[The reason is to avoid different interpretations of the caching, since contrary to FMI for ModelExchange `fmi3DoStep` will perform the actual calculation and not `fmi3GetXXX` and therefore dummy algebraic loops at communication points cannot be handeled by an appropriate sequence of `fmi3GetXXX`, `fmi3SetXXX` calls as for ModelExchange.
